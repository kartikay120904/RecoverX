...
function App() {
  ...
  async function loadDashboard() {
    try {
      ...
      const comparisonResponse = await fetch(`${API}/simulation/compare`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const methodData = Object.entries(report.success_rate_by_method).map(
    ([method, rate]) => ({
  ...
  const recoveryRevenue = report.recovery_recommendations
    .reduce(
    ...
    <div className="app">
      <header className="topbar">
        ...
        <div className="header-actions">
          <span className={`status-dot ${incident.detected ? "danger" : ""}`} />
          {incident.detected ? "Incident Detected" : "System Operational"}
      ...
      <main className="dashboard">
        <section className="hero">
          <div>
            ...
            <p>
              Monitor failures, detect incidents, and prioritize recovery
        ...
        <section className="metrics-grid">
          ...
          <MetricCard
            ...
            detail={`${(metrics.failure_rate * 100).toFixed(2)}% failure rate`}
            danger
          />

          <MetricCard
            label="Failed Volume"
            value={`₹${Number(metrics.failed_volume).toLocaleString()}`}
            detail={`of ₹${Number(metrics.total_volume).toLocaleString()} total volume`}
            danger
          />
          ...
          <div className="panel chart-panel">
            <div className="panel-header">
              <div>
                ...
                <span>Success rate by payment method</span>
              </div>
            </div>
            ...
            <div className="chart">
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={methodData}>
                  ...
                  <YAxis domain={[0, 100]} />
                  <Tooltip />
                  <Bar dataKey="success" name="Success %" radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="panel incident-panel">
            <div className="panel-header">
              <div>
                ...
                <span>Current system assessment</span>
              </div>
            </div>

            <div className={`incident-state ${incident.detected ? "active" : ""}`}>
            ...
            <div className="incident-stats">
              <div>
                ...
                <strong>{(metrics.failure_rate * 100).toFixed(2)}%</strong>
              </div>

              <div>
                <span>Strategy</span>
                <strong>
                  {incident.recommended_strategy.replaceAll("_", " ")}
                </strong>
              </div>
            </div>
          </div>
          ...
          <section className="panel comparison-panel">
            <div className="panel-header">
              <div>
                ...
                <span>Impact of simulated payment infrastructure incidents</span>
              </div>
            </div>
            ...
            <div className="comparison-grid">
              <ComparisonCard
                label="Failure Rate"
                baseline={`${(comparison.baseline.failure_rate * 100).toFixed(2)}%`}
                incident={`${(comparison.incident.failure_rate * 100).toFixed(2)}%`}
                delta={`+${(comparison.impact.failure_rate_delta * 100).toFixed(2)}%`}
              />
              ...
              <ComparisonCard
                label="Failed Volume"
                baseline={`₹${Number(comparison.baseline.failed_volume).toLocaleString()}`}
                incident={`₹${Number(comparison.incident.failed_volume).toLocaleString()}`}
                delta={`+₹${Number(comparison.impact.failed_volume_delta).toLocaleString()}`}
              />
            </div>
          </section>
        )}
        ...
        <section className="panel">
          <div className="panel-header">
            <div>
              ...
              <span>Recommended actions for failed payments</span>
            </div>
              ...
              .map((recommendation) => (
                ...
                >
                  ...
                  <div className="recommendation-value">
                    <strong>
                      {(recommendation.predicted_probability * 100).toFixed(0)}%
                    </strong>
                    <span>
                      ₹{Number(recommendation.predicted_revenue).toLocaleString()}
                    </span>
                  </div>
                </div>
              ))}
...
function ComparisonCard({ label, baseline, incident, delta }) {
  return (
